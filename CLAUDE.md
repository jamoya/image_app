...
Browser → `${VITE_N8N_PROXY_PATH}/webhook/${VITE_N8N_WEBHOOK_ID}` → Vite dev proxy (`vite.config.js`) strips the prefix and forwards to `VITE_N8N_TARGET`. The proxy exists because n8n does not send CORS headers; same-origin requests sidestep that. Production deployments need an equivalent reverse proxy — the frontend never calls n8n cross-origin directly.
